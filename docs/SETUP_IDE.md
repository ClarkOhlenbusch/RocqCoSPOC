...
This guide gets **Rocq** or **Coq** installed and compiling inside Cursor so you can run the [proof workflow](ARCHITECTURE.md): edit `.v` files, see goals and errors, and use the coding agent for tactics.
...
1. In Cursor, use **File → Open Folder** and open the project directory (the one that contains `_CoqProject` and your `.v` files).  
...
After this, you can run the current proof workflow: rewrite the informal proof, generate tactics in the IDE, and use Rocq/Coq errors and goals to iterate quickly.
